...
//
#include "../ColorAdapter/stdafx.h"

#define CATCH_CONFIG_MAIN
#include "../../../catch/catch.hpp"
#include "../ColorAdapter/CModernGraphicsRenderer.h"
#include "../ColorAdapter/CModernGraphicsRendererAdapter.h"
#include "../ColorAdapter/Color.h"
#include "../ColorAdapter/ICanvas.h"
#include "../ColorAdapter/Point.h"
#include "../ColorAdapter/RGBAColor.h"
...
SCENARIO("Adapter")
{
	...
	WHEN("move two times and then draw line")
	{
		...
		const std::string expectedCanvasOutput = CreateExpectedAdapterOutput(secondMovePoint.x, secondMovePoint.y, endLinePoint.x, endLinePoint.y, color);
	...
	WHEN("draw 2 lines")
	{
		...
		const std::string expectedCanvasOutput1 = CreateExpectedAdapterOutput(0, 0, firstLineEndpoint.x, firstLineEndpoint.y, color);
		const std::string expectedCanvasOutput2 = CreateExpectedAdapterOutput(firstLineEndpoint.x, firstLineEndpoint.y, secondLineEndpoint.x, secondLineEndpoint.y, color);
	...
	WHEN("draw line, move and then draw another line")
	{
		...
		const std::string expectedCanvasOutput1 = CreateExpectedAdapterOutput(0, 0, firstPoint.x, firstPoint.y, color);
		const std::string expectedCanvasOutput2 = CreateExpectedAdapterOutput(movePoint.x, movePoint.y, secondPoint.x, secondPoint.y, color);
	...
	WHEN("draw line, set color and then draw another line")
	{
		const modern_graphics_lib ::Point lineEndPoint(1, 1);
		const graphics_lib::Color color = 0xFF00FF;
		const modern_graphics_lib::RGBAColor rgbaColor{ 1, 0, 1, 1 };

		const std::string expectedCanvasOutput = CreateExpectedAdapterOutput(0, 0, lineEndPoint.x, lineEndPoint.y, rgbaColor);

		adapter->SetColor(color);
		adapter->LineTo(lineEndPoint.x, lineEndPoint.y);
